...
PROJECT_NAME = ml-suggest-billing-codes
DATA_S3_PATH := s3://$(PROJECT_NAME)/dvc
...
LOCAL_EXISTS := "$(shell command poetry run pip freeze | grep predict_cpt_codes)"

# Declaring all phony targets (avoid filename/directory collision)
.PHONY: help checkenv init_project download_data create_kernel start_lab lint_project test build_inference_image build_training_image local_train sagemaker_train
...
init_project: checkenv
	poetry init
	pre-commit install
	poetry run dvc remote add -d $(PROJECT_NAME) $(DATA_S3_PATH)
	poetry run dvc add data
	pre-commit install

# Download any data from s3 using DVC
download_data:
	poetry run dvc pull
...
local_train:
	poetry run python -m predict_cpt_codes.modeling.standard.train --input-path data/cli_input --output-path tmp_models

# Run training on SageMaker
sagemaker_train:
	poetry run python -m predict_cpt_codes.modeling.sagemaker.train --image-name 193567999519.dkr.ecr.us-east-1.amazonaws.com/ml-suggest-billing-codes-training:latest \
	--instance-type ml.c5.4xlarge --hyperparameter-file predict_cpt_codes/modeling/sagemaker/hyperparameters.json
